...
README:
v9.0 introduces a completely new method of storing its settings, please make a WP
database backup first (!) and then migrate your settings (cforms will guide you!)
...
function cforms($args = '',$no = '') {
	...
	if ( ($cformsSettings['form'.$no]['cforms'.$no.'_redirect']==2 && isset($_REQUEST['sendbutton'.$no]) && $all_valid) )
		return $content;
	else if ( $cformsSettings['form'.$no]['cforms'.$no.'_maxentries']<>'' && get_cforms_submission_left($no)==0 ){
	...
	if( $cformsSettings['form'.$no]['cforms'.$no.'_action']=='1' ) {
		...
		$action = get_current_page() . '#usermessage'. $no . $actiontarget;
...
function get_cforms_submission_left($no='') {
	...
	if( $max == '' || $cformsSettings['global']['cforms_database']=='0' )
		return -1;
