...
*) bugfix: tracking: fixed filter by form name
...
*) feature: added the possiblity to change the FROM: address to fake the user's
...
function cforms_insert( $content ) {
	...
	if ( ($a=strpos($content,'<!--cforms'))!==false ) {  ### only if form tag is present!
		...
		while( $a !== false ){
			...
			$newcontent .= substr($content,$last,$a-$last);
