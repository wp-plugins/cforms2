...
if( isset($_REQUEST['cforms_rsskeysnew']) ) {
	$cformsSettings['cforms'.$no.'_rsskey'] = md5(rand());
		...
		<fieldset class="cformsoptions" id="anchormessage">
			...
			<div id="o1">
				...
				<div class="optionsbox">
					...
					<div class="optionsboxR">
						<input type="checkbox" id="cforms_errorLI" name="cforms_errorLI" <?php if(substr(get_option('cforms'.$no.'_showpos'),2,1)=="y") echo "checked=\"checked\""; ?>/><label for="cforms_errorLI"><?php _e('Enhanced display of errors (see also new theme CSS classes)', 'cforms'); ?></label>
					</div>
				</div>
				<div class="optionsbox">
		...
		<fieldset class="cformsoptions" id="anchoremail">
			...
			<div id="o2">
				...
				<div class="optionsbox">
					...
					<div class="optionsboxR"><input type="checkbox" id="cforms_rss" name="cforms_rss" <?php if( $cformsSettings['cforms'.$no.'_rss'] ) echo "checked=\"checked\""; ?>/> <?php _e('Enables an RSS feed to track new submissions.', 'cforms'); ?></div>
				</div>

				<?php if( current_user_can('track_cforms') && $cformsSettings['cforms'.$no.'_rss'] ) : ?>
				<div class="optionsbox">
					<div class="optionsboxL"><label for="cforms_rsscount"><strong><?php _e('RSS entries', 'cforms'); ?></strong></label></div>
					<div class="optionsboxR"><input name="cforms_rsscount" id="cforms_rsscount" value="<?php echo $cformsSettings['cforms'.$no.'_rss_count'];  ?>" /></div>
				</div>
				<div class="optionsbox">
					<div class="optionsboxL"><label for="cforms_rsskey"><strong><?php _e('RSS Feed Security Key', 'cforms'); ?></strong></label></div>
					<div class="optionsboxR">
						<input name="cforms_rsskey" id="cforms_rsskey" value="<?php echo $cformsSettings['cforms'.$no.'_rsskey'];  ?>" />
						...
						<br /><?php _e('The complete RSS URL &raquo;', 'cforms'); echo '<br />'.get_option('siteurl').'?cformsRSS='.$no.urlencode('$#$').$cformsSettings['cforms'.$no.'_rsskey']; ?> 
					</div>
				</div>
				<?php endif; ?>
		...
		<fieldset class="cformsoptions" id="commentrep">
			...
			<div id="o7">
				...
				<div class="optionsbox">
					...
					<div class="optionsboxR"><input type="checkbox" id="cforms_commentrep" name="cforms_commentrep" <?php if( substr(get_option('cforms'.$no.'_tellafriend'),0,1)=='2' ) echo "checked=\"checked\""; ?>/><?php _e('Enable this form to optionally (user determined) act as a WP comment form', 'cforms') ?></div>
				</div>
				<div class="optionsbox">
					<div class="optionsboxL"><label for="cforms_commentXnote"><strong><?php _e('Comment notification', 'cforms') ?></strong></label></div>
					<div class="optionsboxR"><input type="checkbox" id="cforms_commentXnote" name="cforms_commentXnote" <?php if( substr(get_option('cforms'.$no.'_tellafriend'),1,1)=='1' ) echo "checked=\"checked\""; ?>/><?php _e('Extra comment notification (check your autoresponder setting, too!)', 'cforms') ?>
