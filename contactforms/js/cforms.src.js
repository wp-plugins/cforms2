...
function call_err(no,err,popFlag){
		...
		//popup error
    err = err.replace(/\\/g,"");
		if ( document.getElementById('_popup'+no).value.charAt(popFlag) == 'y')
					alert( err );
}
	...
	if( doInnerXHTML(msgbox, waiting) ) {
		...
		for (var i = 0, j = objColl.length; i < j; i++) {
				...
				if ( temp.match(/secinput/) )
					newclass = 'secinput';
				else if ( temp.match(/cformchkbox/) )
					newclass = 'cformchkbox';
				    ...
				    if ( temp.match(/required/) ) {
								...
								if ( temp.match(/cformchkbox/) ) {
											...
											if ( objColl[i].checked==false ) {
													...
													if( n.nodeName.toLowerCase()=="label" && !n.className.match(/errortxt/) )
															n.className = n.className + " errortxt";
													else if ( p.nodeName.toLowerCase()=="label" && !p.className.match(/errortxt/) )
		...
		if ( all_valid && upload ){
			document.getElementById('sendbutton'+no).style.display = "none";
			return true;
		}
		else if ( all_valid ) {
		...
		if ( !all_valid && !code_err ){
			call_err(no,document.getElementById('_failure'+no),1);
			return false
		}

		if ( !all_valid ){
			call_err(no,document.getElementById('_codeerr'+no),1);
			return false
		}
