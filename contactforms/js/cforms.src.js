...
// wrapper for reset_captcha		
function x_reset_captcha() {
	sajax_do_call("reset_captcha",
		x_reset_captcha.arguments);
}
...
function cforms_validate(no, upload) {
	...
	var error_container = decodeURIComponent( document.getElementById('cf_customerr'+no).value );
	...
	if( doInnerXHTML(msgbox, waiting) ) {
				    ...
				    if ( temp.match(/required/) ) {
								...
								if ( temp.match(/cf-box-./) ) {
											...
											if ( objColl[i].checked==false ) {
														...
														if ( all_custom_error[objColl[i].id] &&  all_custom_error[objColl[i].id]!='' )
															custom_error = custom_error + '<br />' + all_custom_error[objColl[i].id];
		...
		if ( document.getElementById('cforms_q'+no) && (document.getElementById('cforms_a'+no).value != hex_md5(encodeURI(document.getElementById('cforms_q'+no).value.toLowerCase()) )) ) {
			...
			if ( all_custom_error['cforms_q'+no] &&  all_custom_error['cforms_q'+no]!='' )
				custom_error = custom_error + '<br />' + all_custom_error['cforms_q'+no];
		}
		...
		if ( document.getElementById('cforms_captcha'+no) && (document.getElementById('cforms_cap'+no).value != hex_md5(document.getElementById('cforms_captcha'+no).value.toLowerCase() )) ) {
			...
			if ( all_custom_error['cforms_captcha'+no] &&  all_custom_error['cforms_captcha'+no]!='' )
				custom_error = custom_error + '<br />' + all_custom_error['cforms_captcha'+no];
		}
		...
		for (var i = 0, j = objColl.length; i < j; i++) {
		    ...
		    fld = objColl[i].nodeName.toLowerCase();
				...
				if ( fld == "input" || fld == "textarea" || fld == "select" ) {
						...
						if ( typ == "hidden" && objColl[i].name.match(/cforms_pid/) ) {
...
function cforms_setsuccessmessage(message) {
		...
		if ( message.indexOf('*$#')==-1 && message.match(/http:/))
