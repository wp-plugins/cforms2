	...
	for ( $z=1; $z<= (int) get_option('cforms_formcount'); $z++ ) {
	    ...
	    $j = ($z==1)?'':$z;
		  ...
		  for ( $i=1; $i<=get_option('cforms'.$j.'_count_fields'); $i++)  //now delete all fields from last form
			...
			delete_option('cforms'.$j.'_tellafriend');
		...
		delete_option('cforms_showdashboard');
    	...
    	delete_option('cforms_codeerr');
      	...
      	delete_option('cforms_datepicker');
      	delete_option('cforms_dp_date');
      	delete_option('cforms_dp_days');
      	delete_option('cforms_dp_months');
      	delete_option('cforms_dp_today');
	...
	<form id="cformsdata" name="mainform" method="post" action="">
	 <input type="hidden" name="cforms_database_new" value="<?php if(get_option('cforms_database')=="0") echo 'true'; ?>"/>
		...
		<fieldset id="smtp" class="cformsoptions">
			...
			<div class="optionsbox">
				<div class="optionsboxL"></div>
				<div class="optionsboxR"><input type="checkbox" id="cforms_smtp_onoff" name="cforms_smtp_onoff" <?php if($smtpsettings[0]=="1") echo "checked=\"checked\""; ?>/><label for="cforms_smtp_onoff"><strong><?php _e('Enable specific SMTP server for relaying emails.', 'cforms') ?></strong></label></div>
			</div>
		...
		<fieldset id="visitorv" class="cformsoptions">
			<p class="cflegend"><a class="helptop" href="#top"><?php _e('top', 'cforms'); ?></a><input type="submit" name="Submit1" class="allbuttons updbutton" value="<?php _e('Update Settings &raquo;', 'cforms') ?>" onclick="javascript:document.mainform.action='#visitorv';"/><?php _e('Visitor Verification Settings (Q&A)', 'cforms') ?></p>
		...
		<fieldset id="tracking" class="cformsoptions">
			<p class="cflegend"><a class="helptop" href="#top"><?php _e('top', 'cforms'); ?></a><input type="submit" name="Submit2" class="allbuttons updbutton" value="<?php _e('Update Settings &raquo;', 'cforms') ?>" onclick="javascript:document.mainform.action='#tracking';"/><?php _e('Database Input Tracking', 'cforms') ?></p>
				...
				<div class="optionsbox">
					<div class="optionsboxL"></div>
					<div class="optionsboxR"><input type="checkbox" id="cforms_showdashboard" name="cforms_showdashboard" <?php if(get_option('cforms_showdashboard')=="1") echo "checked=\"checked\""; ?>/><label for="cforms_showdashboard"><strong><?php _e('Show last form submissions on dashboard', 'cforms') ?></strong></label></div>
				</div>
