...
function cforms($args = '',$no = '') {
	...
	for($i = 1; $i <= $field_count; $i++) {
		...
		switch($field_type) {
			...
			case "multiselectbox":
				...
				foreach( $options as $option  ) {
                    ...
                    $field.= $nttt . $tab . '<option value="'. $opt[1] .'"'.$checked.'>'.$opt[0].'</option>';
                    $second = true;
		...
		//close out li item
		if ( ! in_array($field_type,array('hidden','fieldsetstart','fieldsetend','radiobuttons','checkbox','checkboxgroup','ccbox','textonly')) )
			$content .= '</li>';
	...
	$content .= $ntt . '<fieldset class="cf_hidden">';
	...
	if ( $captcha )
		$content .= $nttt .'<input type="hidden" name="cforms_cap'.$no.'" id="cforms_cap'.$no.'" value="' . md5(strtolower($_SESSION['turing_string_'.$no])) . '"/>';

	$custom_error=substr(get_option('cforms'.$no.'_showpos'),2,1).substr(get_option('cforms'.$no.'_showpos'),3,1).$custom_error;
