...
function cforms($args = '',$no = '') {
	...
	for($i = 1; $i <= $field_count; $i++) {
		...
		if($field_emailcheck == 1 && get_option('cforms'.$no.'_emailrequired')<>'' )
			$content .= '<span class="emailreqtxt">&nbsp;'.stripslashes(get_option('cforms'.$no.'_emailrequired')).'</span>';
		else if($field_required == 1 && $field_type <> 'checkbox' && get_option('cforms'.$no.'_required')<>'' )
	...
	if ( get_option('cforms_datepicker')=='1' && !$dpflag ){
		$content .= $indent . $tab . '<div id="datepicker" style="position:absolute;visibility:hidden;z-index:999;"></div><script type="text/javascript">if ( !dp ) dpinit();</script>' . $nl;
		$dpflag   = true;		
	}
...
if (function_exists('add_action')){
	...
	if ( $admin && basename($_SERVER['SCRIPT_FILENAME']) == 'index.php' && get_option('cforms_showdashboard')=='1') {
