...
function cforms_submitcomment($content) {
	...
	for($i = 1; $i <= sizeof($params)-1; $i++) {
			...
			if ( $field_type == "selectbox" || $field_type == "radiobuttons" ) {
			...
			//	$value = rawurldecode($params ['field_' . $i]);
...
function cforms($args = '',$no = '') {
	...
	if( (isset($_POST['sendbutton'.$no]) ) && $all_valid ) {
  		...
  		$to_one = "-1";
			...
			for($i = 1; $i <= $field_count; $i++) {
				...
				if ( $field_type == "selectbox" || $field_type == "radiobuttons" ) { //only needed for field name
				  $field_name = explode('#',$field_name);
				  $field_name = $field_name[0];
				}
	...
	for($i = 1; $i <= $field_count; $i++) {
		...
		if (  in_array($field_type,array('selectbox','radiobuttons','checkbox','ccbox','emailtobox'))  ){
      ...
      $field_name = $options[0];
		...
		if ( ! ($field_type == 'fieldsetstart' || $field_type == 'fieldsetend') && $field_type <> "radiobuttons" && $field_type <> "checkbox" && $field_type <> "ccbox" ) {
		    ...
		    if ( $obj[1] <> '')	$defaultvalue = stripslashes(htmlspecialchars($obj[1]));
				...
				if( $field_type <> 'textonly' )
					$content .= $indent . $tab . $tab . '<label for="'.$label.'"'. $labelclass . '>' . stripslashes(($field_name)) . '</label>';
		}
		...
		switch($field_type) {
			...
			case "textonly":
				$field .= '<p class="textonly' . (($defaultvalue<>'')?' '.$defaultvalue:'') . '" ' . (($reg_exp<>'')?' style="'.$reg_exp.'" ':'') . '>' . stripslashes(htmlspecialchars($field_name)) . '</p>';
				break;

			case "fieldsetstart":
				if ($fieldsetopen) {
						$field = $indent . $tab . '</fieldset>' . $nl;
						$fieldsetopen = false;
				}
				if (!$fieldsetopen) {
						$field .= $indent . $tab .'<fieldset class="cformfieldsets cformfieldset'.$fscount++.'">' . $nl .
											$indent . $tab . '<legend>' . $field_name . '</legend>';
						$fieldsetopen = true;
		 		}
				break;
...
//						$indent = substr($indent,0,-2);
		...
		else if($field_required == 1 && $field_type <> 'checkbox' && $field_type <> 'selectbox' && $field_type <> 'upload' )
	...
	if ( get_option('cforms'.$no.'_ajax')=='1' && !$upload )   // ajax enabled & no upload file field!
	...
	$content .= $indent . $tab . $tab . '<input type="hidden" name="_working'.$no.'" id="_working'.$no.'" value="'.rawurlencode(get_option('cforms'.$no.'_working')).'"/>'. $nl .
				...
				$indent . $tab . $tab . '<input type="hidden" name="_popup'.$no.'"   id="_popup'.$no.'"   value="'.get_option('cforms'.$no.'_popup').'"/>' . $nl;

	$content .= $indent . $tab . $tab . '<input type="submit" name="sendbutton'.$no.'" id="sendbutton'.$no.'" class="sendbutton" value="' . get_option('cforms'.$no.'_submit_text') . '"'.
										$ajaxenabled.'/>' . $nl;
...
function cforms_style() {
	...
	echo "\t".'<link rel="stylesheet" type="text/css" href="' . $cforms_root . '/cforms.css" />'."\n";
