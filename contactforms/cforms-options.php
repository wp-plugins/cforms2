...
if( isset($_REQUEST['Submit1']) || isset($_REQUEST['Submit2']) || isset($_REQUEST['Submit3']) || isset($_REQUEST['Submit4']) || isset($_REQUEST['AddField']) || array_search("X", $_REQUEST) ) {
	...
	for($i = 1; $i <= $field_count; $i++) {
	  ...
	  if ($_REQUEST['field_' . $i . '_name']<>''){    // Newly "AddField" does not exist yet!
				...
				if(isset($_REQUEST['field_' . $i . '_required']) && in_array($type,array('textfield','textarea','checkbox','selectbox','emailtobox','upload')) ) {
					$required = 1;
				}
										...
										<li class="fieldisreq">
											<?php if( !in_array($field_type,array('radiobuttons','fieldsetstart','fieldsetend','ccbox','verification','textonly')) ) {
													?><input class="chkfld" type="checkbox" name="field_<?php echo($i); ?>_required" value="required"  <?php echo($field_required == '1'?' checked="checked"':''); ?> /><?php
														} else if ( !in_array($field_type,array('checkbox','selectbox','radiobuttons','fieldsetstart','fieldsetend','ccbox','verification','textonly')) ) { echo('Required <input type="hidden" name="field_' . $i . '_required" value="required" />'); } ?>
										&nbsp;</li>
										<li class="fieldisemail">
											<?php if( in_array($field_type,array('upload','textarea','checkbox','selectbox','radiobuttons','fieldsetstart','fieldsetend','ccbox','emailtobox','verification','textonly')) ) echo '&nbsp;'; else { ?>
