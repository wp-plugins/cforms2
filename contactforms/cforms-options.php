...
if( isset($_REQUEST['Submit1']) || isset($_REQUEST['Submit2']) || isset($_REQUEST['Submit3']) || isset($_REQUEST['Submit4']) || 
	...
	for($i = 1; $i <= $field_count; $i++) {
	  ...
	  if ($_REQUEST['field_' . $i . '_name']<>''){    // Newly "AddField" does not exist yet!
				...
				if(isset($_REQUEST['field_' . $i . '_required']) && in_array($type,array('textfield','textarea','checkbox','multiselectbox','selectbox','emailtobox','upload','yourname','youremail','friendsname','friendsemail')) ) {
					$required = 1;
				}
				
				if(isset($_REQUEST['field_' . $i . '_emailcheck']) && in_array($type,array('textfield','yourname','youremail','friendsname','friendsemail')) ){
					$emailcheck = 1;
				}

				if(isset($_REQUEST['field_' . $i . '_clear']) && in_array($type,array('textfield','textarea','yourname','youremail','friendsname','friendsemail')) ) {
					$clear = 1;
				}
						...
						<div class="dbx-box">
									...
									<ul class="dbx-cforms<?php echo $alternate?>">
										...
										<li class="fieldtype">
												<select class="selfld" <?php echo $specialclass; ?> name="field_<?php echo($i); ?>_type" id="field_<?php echo($i); ?>_type">
													...
													<?php if ( get_option('cforms'.$no.'_tellafriend') ) : ?>
						            ...
						            ?>
										...
										<li class="fieldisemail">
											<?php if( ! in_array($field_type,array('textfield','yourname','youremail','friendsname','friendsemail')) ) echo '&nbsp;'; else { ?>
										...
										&nbsp;</li>
										<li class="fieldclear">
											<?php if( ! in_array($field_type,array('textarea','textfield','yourname','youremail','friendsname','friendsemail')) ) echo '&nbsp;'; else { ?>
