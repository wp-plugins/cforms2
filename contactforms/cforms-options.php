...
if(isset($_REQUEST['addbutton'])) {
	...
	add_option('cforms'.$no.'_formdata', '11');
...
*/
	...
	update_option('cforms'.$no.'_formdata', ($_REQUEST['cforms_formdata_txt']?'1':'0').($_REQUEST['cforms_formdata_html']?'1':'0') );
...
//										$specialclass = 'style="background:#FAF0AF"';
	...
	<p align="right" style="margin-top:0"><input type="submit" name="Submit2" class="allbuttons updbutton" value="<?php _e('Update Settings &raquo;', 'cforms'); ?>" onclick="javascript:document.mainform.action='#anchorfields';" /></p>
		...
		<fieldset class="cformsoptions" id="anchoremail">
			...
			<div class="optionsbox">
				<div class="optionsboxL"><label for="cforms_subject"><strong><?php _e('Subject', 'cforms') ?></strong></label></div>
			...
			<div class="optionsbox">
				...
				<div class="optionsboxR"><input type="checkbox" id="cforms_formdata_txt" name="cforms_formdata_txt" <?php if(substr(get_option('cforms'.$no.'_formdata'),0,1)=='1') echo "checked=\"checked\""; ?>/><label for="cforms_formdata_txt"><?php _e('Include <em>pre formatted</em> form data table in TXT part', 'cforms') ?></label></div>
			</div>
			<div class="optionsbox">
				<div class="optionsboxL">&nbsp;</div>
			...
			<div class="optionsbox">
				<div class="optionsboxL"></div>
				<div class="optionsboxR"><input type="checkbox" id="cforms_formdata_html" name="cforms_formdata_html" <?php if(substr(get_option('cforms'.$no.'_formdata'),1,1)=='1') echo "checked=\"checked\""; ?>/><label for="cforms_formdata_html"><?php _e('Include <em>pre formatted</em> form data table in HTML part', 'cforms') ?></label></div>
			</div>

			<p class="ex" style="margin-top:30px;"><?php _e('<strong><u>Note</u></strong>: You can disable the inclusion of the <strong>form data</strong> for both the text and html part of the admin email to avoid sending all of the data and instead <strong>configure the message(s)</strong> (TXT & HTML) to include just specific ones (via the use of variables).', 'cforms'); ?></p>
		...
		<fieldset class="cformsoptions" id="autoconf">
			...
			<div class="optionsbox">
				<div class="optionsboxL"><label for="cforms_confirm"><strong><?php _e('Enable', 'cforms') ?></strong></label></div>
			...
			<div class="optionsbox">
				<div class="optionsboxL"><label for="cforms_csubject"><strong><?php _e('Subject', 'cforms') ?></strong></label></div>
			...
			<?php if ( get_option('cforms'.$no.'_cmsg_html')=='' ) : ?>
			<div class="optionsbox">
				<div class="optionsboxL"><?php _e('HTML example<br />you can copy & paste', 'cforms') ?></div>
				<div class="optionsboxRxl">
						<pre><?php echo '&lt;div style="color:#ccc; border-bottom:1px solid #ccc"&gt;'."\n\t".'&lt;strong&gt;'.__('auto confirmation message, {Date}', 'cforms') . "&lt;/strong&gt;\n&lt;/div&gt; \n&lt;br /&gt;\n&lt;p&gt;&lt;strong&gt;" . __('Dear {Your Name},', 'cforms') . "&lt;/strong&gt;&lt;/p&gt;\n&lt;p&gt;". __('Thank you for your note!', 'cforms') . "&lt;/p&gt;\n&lt;p&gt;". __('We will get back to you as soon as possible.', 'cforms') . "&lt;/p&gt;\n"; ?></pre>
				</div>
			</div>
			<?php endif; ?>
		...
		<p align="right"><input type="submit" name="Submit4" class="allbuttons updbutton" value="<?php _e('Update Settings &raquo;', 'cforms'); ?>" onclick="javascript:document.mainform.action='#autoconf';" /></p>
