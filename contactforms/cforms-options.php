...
if(isset($_REQUEST['addbutton'])) {
  ...
  delete_option('cforms'.$FORMCOUNT.'_tellafriend');
  delete_option('cforms'.$FORMCOUNT.'_dashboard');
...
<img src="<?php echo $cforms_root; ?>/images/cfii.gif" alt="" align="right"/><img src="<?php echo $cforms_root; ?>/images/p1-title.jpg" alt=""/>
            ...
            switch ( $field_type ) {
						...
						<div class="dbx-box">
									...
									<ul class="dbx-cforms<?php echo $alternate?>">
										...
										<li class="fieldname"><input class="inpfld" <?php echo $specialclass; ?> name="field_<?php echo($i); ?>_name" id="field_<?php echo($i); ?>_name" size="30" value="<?php echo ($field_type == 'fieldsetend')?'--':$field_name; ?>" /></li>
										<li class="fieldtype">
						            ...
						            ?>
										...
										<li class="fieldisreq">
											<?php if( in_array($field_type,array('multiselectbox','checkboxgroup', 'radiobuttons','fieldsetstart','fieldsetend','ccbox','captcha','verification','textonly')) ) echo '&nbsp;'; else {
										...
										<li class="fieldisemail">
											<?php if( ! in_array($field_type,array('textfield','youremail','friendsemail','email')) ) echo '&nbsp;'; else { ?>
										...
										&nbsp;</li>
										<li class="fieldclear">
											<?php if( ! in_array($field_type,array('textarea','textfield','datepicker','yourname','youremail','friendsname','friendsemail','email','author','url','comment')) ) echo '&nbsp;'; else { ?>
										...
										<li class="fielddisabled">
											<?php if( ! in_array($field_type,array('textarea','textfield','datepicker','checkbox','selectbox','multiselectbox','radiobuttons','upload')) ) echo '&nbsp;'; else { ?>
	...
	<p align="center" style="font-size:11px;margin:20px 0 0;"><em><?php _e('Grab the circle to the left of each field to change the order of the fields (drag&amp;drop)', 'cforms'); ?></em></p>
		...
		<fieldset class="cformsoptions" id="commentrep">
			<p class="cflegend"><a class="helptop" href="#top"><?php _e('top', 'cforms'); ?></a><input type="submit" name="Submit9" class="allbuttons updbutton" value="<?php _e('Update Settings &raquo;', 'cforms'); ?>" onclick="javascript:document.mainform.action='#commentrep';" /><?php _e('WP Comment Feature', 'cforms') ?></p>
			...
			<p><?php echo str_replace('[url]','?page='. $plugindir.'/cforms-help.php#commentrep',__('The forms\' functionality will be extended to cover and thus replace WP\'s comment feature. Check out the <strong><u>5</u></strong> new, comment specific <em>input field types</em> available. <strong>Please see the Help section</strong> for <a href="[url]">more details.</a>', 'cforms')); ?></p>

			<div class="optionsbox">
				<div class="optionsboxL"></label></div>
				<div class="optionsboxR"><input type="checkbox" id="cforms_commentrep" name="cforms_commentrep" <?php if( get_option('cforms'.$no.'_tellafriend')=='2' ) echo "checked=\"checked\""; ?>/><label for="cforms_commentrep"><strong><?php _e('Enable this form to optionally (by the user) act as a WP comment form', 'cforms') ?></strong></div>
				<?php if( get_option('cforms'.$no.'_tellafriend')=='2' ) : ?>
					<div class="optionsboxR">
						<p class="ex">
						<?php echo str_replace('[url]','?page='. $plugindir.'/cforms-help.php#commentrep',__('This feature and T-A-F (above) are mutually exclusive.<br />Again, see the <a href="[url]">help section</a> on proper use.', 'cforms')); ?>
						</p>
					</div>
				<?php endif; ?>
			</div>
