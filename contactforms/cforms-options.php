...
if ( get_option('cforms'.$no.'_showpos')=='' ) {
	...
	<div id="message" class="updated fade"><p><strong>
		<?php echo sprintf(__('It seems that you have recently upgraded cforms, please check the <a href="%s" %s>success/failure message settings</a> and >>Show messages<< options below!', 'cforms'),'#anchormessage','onclick="setshow(1)"') ?>
	</strong></p></div>
...
if ( get_option('cforms'.$no.'_upload_dir')=='' ) {
	...
	<div id="message" class="updated fade"><p><strong>
		<?php echo sprintf(__('In case you\'ve recently upgraded, please check the new <a href="%s" %s>file upload/attachment</a> relevant settings below!', 'cforms'),'#fileupload','onclick="setshow(0)"') ?>
	</strong></p></div>
...
if ( get_option('cforms'.$no.'_header')=='' ) {
	...
	<div id="message" class="updated fade">
		<p><strong><?php echo sprintf(__('It seems that you have recently upgraded cforms, please check the <a href="%s" %s>email header settings</a> below!', 'cforms'),'#anchoremail','onclick="setshow(2)"') ?></strong></p>
		<p><?php _e('If you don\'t know what to insert, here\'s an example:', 'cforms') ?></p>
		<p style="font-family:monospace; padding-left:20px; font-size:11px;"><?php echo sprintf(__('A new form submission %s Submitted on: {Date}<br />Via: {Page}, Visitor IP {IP}%s', 'cforms'),'("{Form Name}")<br />------------------------------------------------<br />','<br />.<br />.') ?></p>
	</div>
...
<img src="<?php echo $cforms_root; ?>/images/cfii.gif" alt="" align="right"/><img src="<?php echo $cforms_root; ?>/images/p1-title.jpg" alt=""/>
	...
	<form enctype="multipart/form-data" id="cformsdata" name="mainform" method="post" action="#anchorfields">
		<input type="hidden" name="noSub" value="<?php echo $noDISP; ?>" />
			...
			<p class="mainoptions">
					<input style="margin-top:0;" type="submit" name="Submit1" class="allbuttons updbutton" value="<?php _e('Update Settings &raquo;', 'cforms') ?>" onclick="javascript:document.mainform.action='#';" />
					<label for="cforms_fname" class="bignumber"><?php _e('Form Name', 'cforms') ?></label>
						<input id="cforms_fname" name="cforms_fname" class="cforms_fname" size="40" value="<?php echo stripslashes(get_option('cforms'.$no.'_fname'));  ?>" />
...
<?php
		...
		$presets  = '<select name="formpresets" id="formpresets">'.get_form_presets().'</select>'.
					'<input id="preset" type="submit" name="preset" class="allbuttons" value="'.__('Install Preset Form', 'cforms').'" onclick="javascript:document.mainform.action=\'#\';return confirm(\'' .__('Do you really want to *replace* all your input fields with this new preset?', 'cforms').'\');" />';

?>
	<fieldset class="cformsoptions" id="anchorfields">
		<p class="cflegend">
			<a class="helptop" href="#top"><?php _e('top', 'cforms'); ?></a>
			<input type="submit" name="Submit2" class="allbuttons updbutton" value="<?php _e('Update Settings &raquo;', 'cforms'); ?>" onclick="javascript:document.mainform.action='#anchorfields';" />
			<?php echo $presets; ?>
			<?php _e('Form Input Fields', 'cforms') ?>
		</p>
		...
		<p>
			<?php echo sprintf(__('Please see the <strong>Help!</strong> section for information on how to deploy the various <a href="%s" %s>supported fields</a>,', 'cforms'),'?page='.$plugindir.'/cforms-help.php#fields','onclick="setshow(19)"') .
					   ...
					   sprintf(__('the use of <a href="%s" %s>default values</a> &amp; <a href="%s" %s>regular expressions</a> (<em>specific field validation; check</em> <code>Required</code>!) for single &amp; multi-line fields.', 'cforms'),'?page='.$plugindir.'/cforms-help.php#single','onclick="setshow(19)"','?page='.$plugindir.'/cforms-help.php#regexp','onclick="setshow(19)"') .
		...
		<p id="cformswarning"><?php echo __('Please save the new order of fields (<em>Update Settings</em>)!','cforms'); ?></p>
		...
		<ul class="tableheader">
			...
			<li class="field4th" title="<?php _e('Makes an input field required for proper form validation.', 'cforms'); ?>"><span class="abbr"><?php _e('Required', 'cforms'); ?></span></li>
			<li class="field5th" title="<?php _e('Makes the field required and verifies the email address.', 'cforms'); ?>"><span class="abbr"><?php _e('E-Mail', 'cforms'); ?></span></li>
	...
	<div id="allfields" class="groupWrapper">
						...
						for($i = 1; $i <= $field_count; $i++) {
								...
								if ( strpos($allfields[$i],'verification') )	$verificationused = true;
								if ( strpos($allfields[$i],'captcha') )			$captchaused = true;
								if ( strpos($allfields[$i],'emailtobox') )		$emailtoboxused = true;
								if ( strpos($allfields[$i],'ccbox') )			$ccboxused = true;
								if ( strpos($allfields[$i],'upload') )			$fileupload = true;
						...
						for($i = 1; $i <= $field_count; $i++) {
								...
								if(sizeof($field_stat) >= 3) {
									...
									add_option('cforms'.$no.'_count_field_' . $i, __('New Field', 'cforms').'$#$textfield$#$0$#$0$#$0$#$0');
								}
						...
						<div id="f<?php echo $i; ?>" class="groupItem<?php echo $alternate; ?>">
							...
							<div class="itemContent">
								...
								<input class="inpfld" <?php echo $specialclass; ?> name="field_<?php echo($i); ?>_name" id="field_<?php echo($i); ?>_name" size="30" value="<?php echo ($field_type == 'fieldsetend')?'--':$field_name; ?>" />
										<select class="fieldtype selfld" <?php echo $specialclass; ?> name="field_<?php echo($i); ?>_type" id="field_<?php echo($i); ?>_type">
						            ...
						            <?php
							          echo '<input '.(($field_count<=1)?'disabled="disabled"':'').' class="'.(($field_count<=1)?'noxbutton':(($alternate<>'')?'altx':'xbutton')).'" type="submit" name="DeleteField'.$i.'" value="" title="'.__('Remove input field', 'cforms').'" alt="'.__('Remove input field', 'cforms').'" onfocus="this.blur()"/>';
						            ?>
								
									<?php if( in_array($field_type,array('multiselectbox','checkboxgroup', 'radiobuttons','fieldsetstart','fieldsetend','ccbox','captcha','verification','textonly')) ) : ?>
										<img class="chkno" src="<?php echo $cforms_root; ?>/images/chkbox_grey.gif" alt="<?php _e('n/a', 'cforms'); ?>" title="<?php _e('Not available.', 'cforms'); ?>"/>
									<?php else : ?>
										<input class="fieldisreq chkfld" type="checkbox" name="field_<?php echo($i); ?>_required" value="required"  <?php echo($field_required == '1'?' checked="checked"':''); ?> />								
									<?php endif; ?>

									<?php if( ! in_array($field_type,array('textfield','youremail','friendsemail','email')) ) : ?>
										<img class="chkno" src="<?php echo $cforms_root; ?>/images/chkbox_grey.gif" alt="<?php _e('n/a', 'cforms'); ?>" title="<?php _e('Not available.', 'cforms'); ?>"/>
									<?php else : ?>
										<input class="fieldisemail chkfld" type="checkbox" name="field_<?php echo($i); ?>_emailcheck" value="required"  <?php echo($field_emailcheck == '1'?' checked="checked"':''); ?> />
									<?php endif; ?>
									

									<?php if( ! in_array($field_type,array('textarea','textfield','datepicker','yourname','youremail','friendsname','friendsemail','email','author','url','comment')) ) : ?>
										<img class="chkno" src="<?php echo $cforms_root; ?>/images/chkbox_grey.gif" alt="<?php _e('n/a', 'cforms'); ?>" title="<?php _e('Not available.', 'cforms'); ?>"/>
									<?php else : ?>
										<input class="fieldclear chkfld" type="checkbox" name="field_<?php echo($i); ?>_clear" value="required"  <?php echo($field_clear == '1'?' checked="checked"':''); ?> />
									<?php endif; ?>

									<?php if( ! in_array($field_type,array('textarea','textfield','datepicker','checkbox','checkboxgroup','selectbox','multiselectbox','radiobuttons','upload')) ) : ?>
										<img class="chkno" src="<?php echo $cforms_root; ?>/images/chkbox_grey.gif" alt="<?php _e('n/a', 'cforms'); ?>" title="<?php _e('Not available.', 'cforms'); ?>"/>
									<?php else : ?>
										<input class="fielddisabled chkfld" type="checkbox" name="field_<?php echo($i); ?>_disabled" value="required"  <?php echo($field_disabled == '1'?' checked="checked"':''); ?> />
									<?php endif; ?>
		...
		<div class="addfieldbox"><input type="submit" name="AddField" class="addfield" title="<?php _e('Add another input field', 'cforms'); ?>" value=" " onfocus="this.blur()" onclick="javascript:document.mainform.action='#anchorfields';" /></div>
			...
			<div id="o1">
				...
				<div class="optionsbox" style="margin-top:10px;">
					...
					<div class="optionsboxR">
						<textarea rows="80px" cols="200px" style="float:left" name="cforms_success" id="cforms_success"><?php echo stripslashes(htmlspecialchars(get_option('cforms'.$no.'_success'))); ?></textarea>
				...
				<div class="optionsbox">
					<div class="optionsboxL"><label for="cforms_failure"><?php _e('<strong>Failure message</strong><br />missing fields or wrong field formats<br />(regular expr.)', 'cforms'); ?></label></div>
						<div class="optionsboxR"><textarea rows="80px" cols="200px" style="float:left" name="cforms_failure" id="cforms_failure" ><?php echo stripslashes(htmlspecialchars(get_option('cforms'.$no.'_failure'))); ?></textarea>
			...
			<div id="o2">
				...
				<?php if( get_option('cforms_showdashboard') == '1' ) : ?>
					<div class="optionsbox" style="margin-top:15px;">
						...
						<div class="optionsboxR"><input type="checkbox" id="cforms_dashboard" name="cforms_dashboard" <?php if($o=get_option('cforms'.$no.'_dashboard')=="1") echo "checked=\"checked\""; ?>/><label for="cforms_dashboard"><strong><?php _e('Include in last 5 entries shown on the dashboard.', 'cforms') ?></strong></label></div>
					</div>
				<?php endif; ?>
		...
		<fieldset class="cformsoptions" id="emailoptions">
			...
			<div id="o3">
				...
				<div class="optionsbox" style="margin-top:30px;">
					...
					<div class="optionsboxRxl"><textarea rows="80px" cols="200px" name="cforms_header" id="cforms_header" ><?php echo stripslashes(htmlspecialchars(get_option('cforms'.$no.'_header'))); ?></textarea> <?php echo sprintf(__('<a href="%s" %s>Variables</a> allowed.', 'cforms'),'?page='. $plugindir.'/cforms-help.php#variables','onclick="setshow(23)"'); ?></div>
				</div>
				<div class="optionsbox">
					<div class="optionsboxL"></div>
				...
				<div class="optionsbox <?php if( !$o=='1' ) echo "hidden"; ?>">
					...
					<div class="optionsboxRxl"><textarea rows="80px" cols="200px" name="cforms_header_html" id="cforms_header_html" ><?php echo stripslashes(htmlspecialchars(get_option('cforms'.$no.'_header_html'))); ?></textarea> <?php echo sprintf(__('<a href="%s" %s>Variables</a> allowed.', 'cforms'),'?page='. $plugindir.'/cforms-help.php#variables','onclick="setshow(23)"'); ?></div>
				</div>
				<div class="optionsbox <?php if( !$o=='1' ) echo "hidden"; ?>">
					<div class="optionsboxL"></div>
				...
				<p class="ex" style="margin-top:30px;"><strong><u><?php _e('Note:','cforms')?></u></strong> <?php _e('To avoid sending ALL of the submitted user data (especially for very long forms) to the form admin simply <strong>uncheck</strong> "<em>Include pre formatted form input ...</em>" and instead specify the fields you\'d like to receive via the use of <strong>custom variables</strong>.', 'cforms'); ?></p>
		...
		<fieldset class="cformsoptions" id="autoconf">
			...
			<div id="o5">
				...
				<div class="optionsbox <?php if( !$o=="1" ) echo "hidden"; ?>" style="margin-top:10px;">
					...
					<div class="optionsboxRxl"><textarea rows="80px" cols="200px" name="cforms_cmsg" id="cforms_cmsg" ><?php echo stripslashes(htmlspecialchars(get_option('cforms'.$no.'_cmsg'))); ?></textarea>  <?php echo sprintf(__('<a href="%s" %s>Variables</a> allowed.', 'cforms'),'?page='. $plugindir.'/cforms-help.php#variables','onclick="setshow(23)"'); ?></div>
				</div>
				...
				<div class="optionsbox <?php if( !$o=="1" || !$o2=="1") echo "hidden"; ?>">
					...
					<div class="optionsboxRxl"><textarea rows="80px" cols="200px" name="cforms_cmsg_html" id="cforms_cmsg_html" ><?php echo stripslashes(htmlspecialchars(get_option('cforms'.$no.'_cmsg_html'))); ?></textarea> <?php echo sprintf(__('<a href="%s" %s>Variables</a> allowed.', 'cforms'),'?page='. $plugindir.'/cforms-help.php#variables','onclick="setshow(23)"'); ?></div>
				</div>
