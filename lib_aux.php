...
function get_captcha_uri() {
	...
	$c1 = prep( $captcha['c1'],'3' );
	$c2 = prep( $captcha['c2'],'5' );
	$ac = prep( $captcha['ac'],'abcdefghijkmnpqrstuvwxyz23456789' );
	$i = prep( $captcha['i'],'' );
	$h = prep( $captcha['h'],25 );
...
class cformsRSS {
	...
	function outputRSS() {
		...
		if( $temp <> '' ) {
			...
			$all = $cformsRSS[0]=='-1' && $cformsSettings['cforms_rssall'] && $cformsSettings['cforms_rsskeyall'] == $cformsRSS[1];
			$single = $cformsSettings['cforms'.$cformsRSS[0].'_rss'] && $cformsRSS[1]<>'' && $cformsSettings['cforms'.$cformsRSS[0].'_rsskey'] == $cformsRSS[1];
			if( $all || $single ){
				...
				$WHERE='';
				if( $all )
					$rsscount = $cformsSettings['cforms_rssall_count'];
				else if( $single ){
					$WHERE = "WHERE form_id = '".$cformsRSS[0]."'";
					$rsscount = $cformsSettings['cforms'.$cformsRSS[0].'_rss_count'];
				}
				...
				if( count($entries)>0 ){
					foreach($entries as $entry){
							$content.= "\t".'<item>'."\n".
										...
										"\t\t".'<description>'.__('Form submitted on','cforms').' '.$entry->sub_date.($single?'':' via "'.get_option('cforms'.$entry->form_id.'_fname').'"').'.</description>'."\n".
										"\t\t".'<link>'.get_option('siteurl').'wp-admin/admin.php?page='.$plugindir.'/cforms-database.php#e'.$entry->id.'</link>'."\n".							
										"\t\t".'<guid isPermaLink="false">'.get_option('siteurl').'/wp-admin/admin.php?page='.$plugindir.'/cforms-database.php#e'.$entry->id.'</guid>'."\n".
										"\t\t".'<pubDate>'.mysql2date('D, d M Y H:i:s +0000', $entry->sub_date, false).'</pubDate>'."\n".
										//'<pubDate><content:encoded><![CDATA['.'XXX'.']]></content:encoded></pubDate>'.
										"\t".'</item>'."\n";
					}
				}
				else
...
<channel>
	<title><?php if ($single) echo __('New submissions for &raquo;', 'cforms').' '.get_option('cforms'.$no.'_fname'); else _e('All new form submissions', 'cforms'); ?></title>
	<atom:link href="<?php echo get_option('siteurl').'?cformsRSS='.$no.urlencode('$#$').$cformsSettings['cforms'.$no.'_rsskey']; ?>" rel="self" type="application/rss+xml" />
