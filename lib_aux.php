...
function get_cforms_entries($fname=false,$from=false,$to=false,$sort=false,$limit=false) {
	...
	$where = $fname_in?' form_id IN ('.substr($fname_in,0,-1).')':'';
				...
				if( count($entries)>0 ){
					foreach($entries as $entry){
							$content.= "\t".'<item>'."\n".
										...
										"\t\t".'<description>'.__('Form submitted on','cforms').' '.$entry->sub_date.($single?'':' via "'.$cformsSettings['form'.$entry->form_id]['cforms'.$entry->form_id.'_fname'].'"').'.</description>'."\n".
...
<channel>
	<title><?php if ($single) echo __('New submissions for &raquo;', 'cforms').' '.$cformsSettings['form'.$no]['cforms'.$no.'_fname']; else _e('All new form submissions', 'cforms'); ?></title>
