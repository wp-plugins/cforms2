...
function cforms_submitcomment($content) {
	...
	for($i = 1; $i <= sizeof($params)-2; $i++) {
			...
			$jump = in_array($field_stat[1],array('captcha','verification')) && is_user_logged_in();

			// filter non input fields
			while ( in_array($field_stat[1],array('fieldsetstart','fieldsetend','textonly')) || ($jump) ) {
...
function ajaxMail($isAjaxWPcomment,$no,$field_email,$formdata,$htmlformdata,$taf_youremail,$taf_friendsemail,$taf_yourname,$taf_youremail,$ccme,$to_one,$segments){
	...
	// T-A-F override?
	if ( substr(get_option('cforms'.$no.'_tellafriend'),0,1)=='1' && $taf_youremail && $taf_friendsemail )
