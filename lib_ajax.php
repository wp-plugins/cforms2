...
function cforms_submitcomment($content) {
	global $cformsSettings, $wpdb, $subID, $styles, $smtpsettings, $track, $Ajaxpid, $AjaxURL, $wp_locale, $abspath;
	...
	// form limit reached
	if ( $cformsSettings['form'.$no]['cforms'.$no.'_maxentries']<>'' && get_cforms_submission_left($no)==0 ){
	...
	if ( $isAjaxWPcomment!==false && $track['send2author']=='0' ){
		...
		if ($WPsuccess){
			...
			if      ( $cformsSettings['form'.$no]['cforms'.$no.'_redirect']==1 ) return $cformsSettings['form'.$no]['cforms'.$no.'_redirect_page'];
...
function ajaxMail($isAjaxWPcomment,$no,$field_email,$formdata,$htmlformdata,$taf_youremail,$taf_friendsemail,$taf_yourname,$taf_youremail,$ccme,$to_one,$segments){
	...
	global $wpdb, $subID, $styles, $smtpsettings, $track, $Ajaxpid, $AjaxURL, $wp_locale, $cformsSettings;
